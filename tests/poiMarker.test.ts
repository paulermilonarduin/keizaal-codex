...
describe('buildPoiMarkerHtml (#81)', () => {
  ...
  test('la variable CSS ne bouge pas au survol', () => {
    const base = { editable: false, hovered: false, movable: false }
  ...
  test('l’étiquette est toujours rendue, quel que soit l’état des autres options (#127)', () => {
    const base = { editable: false, hovered: false, movable: false }

    const html = buildPoiMarkerHtml(makePoi(), base)
    assert.match(html, /poi-label/)
    assert.match(html, /Blancherive/)

    assert.match(buildPoiMarkerHtml(makePoi(), { ...base, hovered: true }), /poi-label/)
    assert.match(buildPoiMarkerHtml(makePoi(), { ...base, editable: true }), /poi-label/)
    assert.match(buildPoiMarkerHtml(makePoi(), { ...base, movable: true }), /poi-label/)
  })

  test('le repère est toujours rendu (#68)', () => {
    const html = buildPoiMarkerHtml(makePoi(), {
      editable: false,
      hovered: false,
      movable: false,
